...
use dojoswap::mock_querier::{mock_dependencies, WasmMockQuerier};
...
fn create_pair() {
    ...
    deps = init(deps);
    deps.querier
        .with_dojoswap_factory(&[], &[("uusd".to_string(), 6u8)]);
    let assets = [
        Asset {
...
fn fail_to_create_pair_with_unknown_token() {
    ...
    deps.querier
        .with_dojoswap_factory(&[], &[("uluna".to_string(), 6u8)]);
...
fn reply_only_create_pair() {
    ...
    // register dojoswap pair querier
    deps.querier.with_dojoswap_factory(
        &[(
            &"0000".to_string(),
            &PairInfo {
