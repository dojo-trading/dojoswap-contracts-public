...
use dojo_staking_helpers::staking::{
...
pub fn instantiate(
    ...
    store_config(
        deps.storage,
        &Config {
            dojo_token: deps.api.addr_canonicalize(&msg.dojo_token)?,
...
pub fn query_config(deps: Deps) -> StdResult<ConfigResponse> {
    ...
    let resp = ConfigResponse {
        dojo_token: deps.api.addr_humanize(&state.dojo_token)?.to_string(),
