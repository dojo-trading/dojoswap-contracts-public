...
use dojoswap::querier::{query_pair_info, reverse_simulate, simulate};
use dojoswap::router::{
...
const CONTRACT_NAME: &str = "crates.io:dojoswap-router";
...
pub fn instantiate(
    ...
    CONFIG.save(
        deps.storage,
        &Config {
            dojoswap_factory: deps.api.addr_canonicalize(&msg.dojoswap_factory)?,
        },
    )?;
...
pub fn query_config(deps: Deps) -> StdResult<ConfigResponse> {
    ...
    let resp = ConfigResponse {
        dojoswap_factory: deps
            .api
            .addr_humanize(&state.dojoswap_factory)?
            .to_string(),
    };
...
fn simulate_swap_operations(
    ...
    let dojoswap_factory = deps.api.addr_humanize(&config.dojoswap_factory)?;
    ...
    for operation in operations.into_iter() {
        match operation {
            SwapOperation::DojoSwap {
                offer_asset_info,
                ask_asset_info,
            } => {
                let pair_info: PairInfo = query_pair_info(
                    &deps.querier,
                    dojoswap_factory.clone(),
